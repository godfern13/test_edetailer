$(document).ready(function(){
	...
	$("#createArea").click(function(){showParSpec()});
	var offset2 = $("#createArea").offset();
	var w = $(window);
	...
	$(".addTool").draggable({
		helper:'clone',
		containment: 'createArea',
		/*--------------------------------------- first time drag -----------------------------------------------*/
		stop:function(ev, ui) { 
			/********** CHANGE 17/09/2013 START **************************/
			if(ui.helper.attr('id') == 'textbox'){ var childType=1;}
			else if(ui.helper.attr('id') == 'imgbox'){ var childType=2;}
			else if(ui.helper.attr('id') == 'videobox'){ var childType=3;}
			...
			$(objName).draggable({
				...
				//alert(parentX+'='+parentY+'='+counter+'='+childType);
...
{ 
	...
	var parentWdth	=	$("#createArea").width();
	var parentHght	=	$("#createArea").height();
	var parentBgClr	=	$("#createArea").css("background-color");
	...
	$.ajax({
		...
		success: function(data) { 
			$('#displaySpecifications').html(data);
			$('#specLoader').hide();
...
function chngeParSpec()
{
	...
	var parentName	=	$("#sepcName").val();
	var pBgImgName	=	$("#pBgImgName").val();
	if(pBgImgName != ""){ $('#createArea').css("background","url('images/parent/"+pBgImgName+"')no-repeat"); }
	else{$("#createArea").css("background",bColor);}
...
function ParntBgImgURL(upload_field) { 
	if (upload_field.files && upload_field.files[0]) {
		...
		reader.onload = function (e) { 
			$('#createArea').css("background","url('"+e.target.result+"')no-repeat");
		};
...
/*---------------------------------------------- Upload Image To Server Script --------------------------------*/
	...
	if (filename.search(re_text) == -1) {
		...
		var bColor		=	$('#sepcBgColor').val();
		$("#createArea").css("background",bColor);
		return false;
	}
